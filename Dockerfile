...
# docker build -t rag_data_proyecto .
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name rag_data_proyecto rag_data_proyecto
...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y curl libjemalloc2 postgresql-client poppler-utils && \
...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y build-essential git libpq-dev libyaml-dev pkg-config libffi-dev && \
    rm -rf /var/lib/apt/lists /var/cache/apt/archives
